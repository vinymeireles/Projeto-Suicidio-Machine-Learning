...
st.markdown("""<h6 style='text-align: justify;'> Essa aplicação demonstra uma análise exploratória dos dados de "Suicídios" ocorridos pelo mundo. Todos os anos, mais de 12 mil pessoas tiram suas próprias vidas no Brasil. 
...
st.markdown("""<h6 style='text-align: justify;'> Segundo a Organização Mundial da Saúde (OMS), o Brasil ocupa o oitavo lugar no número de suicídios no mundo: São 32 brasileiros por dia. Animações gráficas e previsões futuras de novos casos utilizando IA serão mostradas nas seções "Brasil" e "Global" e interpretadas os seus resultados. **Setembro Amarelo** é uma iniciativa da Associação Brasileira de Psiquiatria (ABP), em parceria com o Conselho Federal de Medicina (CFM), para divulgar e alertar a população sobre o problema. Oficialmente, o Dia Mundial de Prevenção ao Suicídio ocorre no dia 10 de setembro, porém durante o mês inteiro são promovidos debates, campanhas e ações para a conscientização sobre o suicídio.
...
st.markdown("""<h6 style='text-align: justify;'> Assim, tendo em mente os fatos acima, essa aplicação usa algoritmos de Machine Learning (IA) para prever as taxas de suicídio futuras ao analisar e encontrar sinais correlacionados ao aumento das taxas de suicídio entre diferentes países em todo o espectro socioeconômico. O conjunto de dados usado é fornecido pela https://www.kaggle.com/ e https://ourworldindata.org/suicide. </h6""", unsafe_allow_html=True)
